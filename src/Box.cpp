...
void Box::initProgram(const glimac::FilePath &vsFile, const glimac::FilePath &fsFile)
{
...
void Box::addGroundAndFront(const glm::vec3 &cameraPos, std::vector<glimac::BBox3f> &bboxVector)
{
...
void Box::addRightAndLeft(const glm::vec3 &cameraPos, float order, std::vector<glimac::BBox3f> &bboxVector)
{
    auto vertices = Quad::QuadVertices(1, 1);
    auto transformMat = glm::mat4(1);
